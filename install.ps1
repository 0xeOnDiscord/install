param (
  [Parameter()]
  [switch]
  $UninstallSpotifyStoreEdition = (Read-Host -Prompt 'Desinstaller les pubs de merde spotify (Y/N)') -eq 'y',
  [Parameter()]
  [switch]
  $UpdateSpotify,
  [Parameter()]
  [switch]
  $RemoveAdPlaceholder = (Read-Host -Prompt 'Desinstaller les skips de merde spotify (Y/N)') -eq 'y'
)
...
function Get-File
{
  ...
  else
  {
    ...
    while ($count -gt 0)
    {
      ...
      Write-Progress -Activity "Telechargement du fichier... '$downloadedFileName'" -Status "Downloaded ($([System.Math]::Floor($downloadedBytes/1024))K of $($totalLength)K): " -PercentComplete ((([System.Math]::Floor($downloadedBytes / 1024)) / $totalLength) * 100)
    }

    Write-Progress -Activity "Finis ! '$downloadedFileName'"
...
if (Get-AppxPackage -Name SpotifyAB.SpotifyMusic)
{
  ...
  {
    Write-Host "Casse un tour spotify.`n"
...
Write-Host "Telechargement des ressources...`n"
...
{
  if ((Read-Host -Prompt 'Voulez-vous vraiment lancer le programme (vous serez deconnecte de votre compte spotify)? (Y/N)') -ne 'y')
  {
    exit
  }
}
...
{
  Write-Host 'Telechargement des ressources...'
...
Write-Host 'Lancement de spotify ...'
